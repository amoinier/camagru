...
if (isset($postres['post']) && $postres['post'] == 1) {
	if (htmlspecialchars($_POST['like']) === 'like' && is_numeric($pid)) {
		...
		$findmail = $bdd->query('SELECT * FROM `users` WHERE `login` LIKE "'.htmlspecialchars($postres['login']).'";');
		$findmailre = $findmail->fetch();
		$headers = 'From: InstaSnap'."\r\n";
		$headers.= "MIME-version: 1.0\n";
		$headers.= "Content-type: text/html; charset= iso-8859-1\n";
		$message = "Hi ".htmlspecialchars($postres['login']).", <html><body>You receive a new commentary, click <a href='http://localhost:8080".substr($_SERVER['PHP_SELF'], 0, strlen($_SERVER['PHP_SELF']) - strlen(strrchr($_SERVER['PHP_SELF'], "/")))."/photo.php?login=".$postres['login']."&idphoto=".$pid."'>here</a> to see it!</html></body>";
